...
const [onFocus, setOnFocus] = useState(false)
  ...
  const handleGetCurrentAddress = () => {
    ...
    getCurrentAddress().then(async (data) => {
      const dataAddress = data?.address?.features?.[0]?.properties
      if (!dataAddress) return toast.error("Get failed")
      const address = {
      province: dataAddress?.city,
      district: dataAddress?.county,
      ward: '',
      street: dataAddress?.street,
      lat:dataAddress?.lat,
      lng: dataAddress?.lon,
      address_text: `${dataAddress?.address_line1} ${dataAddress?.county} ${dataAddress?.city} ${dataAddress?.country}`
      };
    ...
    >
      ...
      >
        ...
        >
          ...
          {user?.address ? (
            <Typography color={"white"} fontWeight={600} fontSize={16}>
              {user?.address?.address_text ||address(user.address)}
            </Typography>
          ) : (
            <Skeleton
              variant="text"
        ...
        >
          ...
          />
          {onFocus && searchHistory?.length && searchQuery !== "" ? (
            <Box
              sx={{
                position: "absolute",
                top: 50,
